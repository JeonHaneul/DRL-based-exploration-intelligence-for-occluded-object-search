...
# Config class
class Config:
    TEST_DIR = "./test_250422"
    OUTPUT_DIR = "./outputs"
    MODEL_PATH = os.path.join(OUTPUT_DIR, "best_model_45.pth")
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transforms:
            ...
            # print(f"Debug: Normalized image: min={img.min()}, max={img.max()}")
        else:
            img = torch.tensor(img, dtype=torch.float32)
...
def predict_save_and_generate_distribution(model, data_loader, target_class_idx, device, target_class):
    ...
    gain = 2.0  # 지수 함수의 gain 값 (0.2)
    gamma = 0.7  # 이전 분포와 현재 분포의 가중치를 위한 값(현재 확율을 얼마나 반영할지) (0.7)
